			...
			<scroll-view :scroll-y="true" :style="{height: 'calc(100vh - ' +  (barHeight*2 + 280) + 'rpx)'}">
				...
				<view class="file-box" v-for="(item,index) in fileArr" @click="selectFile(index)">
					<view class="file-name-box">
						...
						<image :src="videoImg" mode="widthFix" class="file-img" v-if="item.type == 'mp4'"></image>
						<image :src="audioImg" mode="widthFix" class="file-img" v-if="item.type == 'mp3'"></image>
						<image :src="imageImg" mode="widthFix" class="file-img" v-if="item.type == 'image'"></image>
						<view class="name-box">
							{{item.name}}
						</view>
					</view>
...
*/
	...
	export default {
		...
		props:{
			...
			backImg:{
				type: String,
				default: require("./icons/back.png")
			},
			directionImg:{
				type: String,
				default: require("./icons/direction.png")
			},
			enterImg:{
				type: String,
				default: require("./icons/enter.png")
			},
			folderImg:{
				type: String,
				default: require("./icons/folder.png")
			},
			fileImg:{
				type: String,
				default: require("./icons/file.png")
			},
			txtImg:{
				type: String,
				default: require("./icons/txt.png")
			},
			docImg:{
				type: String,
				default: require("./icons/docx.png")
			},
			pdfImg:{
				type: String,
				default: require("./icons/pdf.png")
			},
			videoImg:{
				type: String,
				default: require("./icons/video.png")
			},
			audioImg:{
				type: String,
				default: require("./icons/audio.png")
			},
			imageImg:{
				type: String,
				default: require("./icons/image.png")
			},
			selectedImg:{
				type: String,
				default: require("./icons/home_icon_selected.png")
			},
			unselectedImg:{
				type: String,
				default: require("./icons/home_icon_unselected.png")
			},
			titel:{
				type: String,
				default: "选择文件"
				...
				for(let i = 0; i < len; i++){
					...
					if(!plus.android.invoke(files[i],"isHidden")){
						...
						if(plus.android.invoke(files[i],"isDirectory")){ 
							...
							else if(fileName.search(/mp4/i) > -1){
								// mp4 文件
								this.fileArr.push({name: fileName,file: files[i],type: "mp4",select: false});
							}
							else if(fileName.search(/mp3/i) > -1){
								// mp3 文件
								this.fileArr.push({name: fileName,file: files[i],type: "mp3",select: false});
							}
							else if(fileName.search(/\.(jpg|jpeg|png)/i) > -1){
								// image 文件
								this.fileArr.push({name: fileName,file: files[i],type: "image",select: false});
							}
							else{
								// 其他文件
